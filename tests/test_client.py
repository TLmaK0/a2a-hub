...
from conftest import AGENT_A, AGENT_B, TOKEN_A, TOKEN_B
...
async def make_client(app):
    ...
    def _factory(agent: str, token: str) -> HubClient:
        ...
        config = ClientConfig(url="/", agent=agent, token=token)
...
async def test_client_send_and_receive(make_client):
    ...
    result = await run(sender.send_message, AGENT_B, "hello over the wire")
    ...
    assert artifact["metadata"]["sender"] == AGENT_A


async def test_client_mailboxes_are_isolated(make_client):
    sender = make_client(AGENT_A, TOKEN_A)
    await run(sender.send_message, AGENT_B, "for B only")
...
async def test_client_get_task(make_client):
    sender = make_client(AGENT_A, TOKEN_A)
    recipient = make_client(AGENT_B, TOKEN_B)
    sent = await run(sender.send_message, AGENT_B, "fetch me")
...
async def test_cli_send_then_inbox(make_client, capsys):
    ...
    assert await run(main, ["send", AGENT_B, "cli", "message"], client=sender) == 0
    assert "COMPLETED" in capsys.readouterr().out

    assert await run(main, ["inbox"], client=recipient) == 0
    out = capsys.readouterr().out
    assert f"mailbox of {AGENT_B}" in out
    assert "cli message" in out
    assert f"from {AGENT_A}" in out


async def test_cli_inbox_json_and_read(make_client, capsys):
    sender = make_client(AGENT_A, TOKEN_A)
    recipient = make_client(AGENT_B, TOKEN_B)
    sent = await run(sender.send_message, AGENT_B, "json please")
    task_id = sent["task"]["id"]
...
def test_config_from_env(tmp_path):
    ...
    assert (cfg.url, cfg.agent, cfg.token) == ("https://h/", "me", "t")
...
def test_config_env_overrides_file(tmp_path):
    path = tmp_path / "agent.env"
    path.write_text("A2A_HUB_URL=https://f/\nA2A_HUB_AGENT=file\nA2A_HUB_TOKEN=ft\n")
...
def test_cli_reports_config_error(tmp_path, monkeypatch, capsys):
    ...
    for var in ("A2A_HUB_URL", "A2A_HUB_AGENT", "A2A_HUB_TOKEN"):
...
def test_client_uses_urllib_transport_by_default():
    ...
    hub = HubClient(ClientConfig(url="https://h/", agent="a", token="t"))
